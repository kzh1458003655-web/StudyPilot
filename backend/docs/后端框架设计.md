...
> 实施状态（2026-09-08）：`HARNESS-001`、`ARCH-001` 至 `ARCH-004` 与 `DB-001` 已验证完成。当前进入基础集成验证；跨模块决定和迁移记录统一放在 [关键记录](../../docs/关键记录/README.md)。
...
本文档位于 `backend/docs/后端框架设计.md`，是后端骨架的架构基线，并随工程一同维护。
...
Spring Boot只通过PostgreSQL JDBC连接Supabase，不使用前端SDK或Data API。数据库主机、用户名、密码、项目标识和任何连接串均只保存在本机环境变量或受控密钥管理系统中，禁止写入仓库、文档、测试输出或Issue。
...
`DB-001` 已将以下表和最小字段落实为可执行 DDL；业务功能开始后新增字段、状态或约束时必须通过显式迁移记录，不得修改旧脚本并假设线上数据库自动更新。完整关系、字段目的、验证与本地运行方式见 [数据库实现说明](数据库实现说明.md)。
