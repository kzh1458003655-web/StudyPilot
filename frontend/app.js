...
createApp({
 ...
 methods:{
  ...
  // 统计数据与答卷历史随课程切换；题目答案不在此接口返回。
  async refreshAssessment(){const epoch=this.courseEpoch;try{const [topics,attempts]=await Promise.all([this.api(this.scoped('/assessment/topics')),this.api(this.scoped('/assessment/attempts'))]);if(epoch===this.courseEpoch){this.examTopics=topics;this.examAttempts=attempts}}catch(e){if(epoch===this.courseEpoch)this.notify(e.message)}},
  async seedAssessment(){if(this.seeding)return;this.seeding=true;try{const result=await this.api('/assessment/seed',this.json('POST',{courseId:this.courseId}));await this.refreshAssessment();this.notify(result.created?'演示题库已载入':'当前课程已有题库','success')}catch(e){this.notify(e.message)}finally{this.seeding=false}},
  async createPaper(){if(this.paperLoading)return;this.paperLoading=true;try{this.paper=await this.api('/assessment/papers',this.json('POST',{courseId:this.courseId,count:5}));this.examAnswers={};this.examResult=null;this.tab='assessment';this.notify('已生成一份模拟测验','success')}catch(e){this.notify(e.message)}finally{this.paperLoading=false}},
  optionLabel(index){return String.fromCharCode(65+index)},
  async submitPaper(){if(!this.paper||this.submitting)return;const answers=this.paper.questions.map(q=>({question_id:q.id,answer:(this.examAnswers[q.id]||'').trim()}));if(answers.some(a=>!a.answer))return this.notify('请完成全部题目后再提交');this.submitting=true;try{this.examResult=await this.api('/assessment/papers/'+this.paper.id+'/submit',this.json('POST',{courseId:this.courseId,answers}));await this.refreshAssessment();this.notify('测验已完成，评分结果已保存','success')}catch(e){this.notify(e.message)}finally{this.submitting=false}},
