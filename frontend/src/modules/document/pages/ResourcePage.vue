...
async function submit() {
  ...
  try {
    ...
    notice.value = `处理完成：已提取 ${result.pageCount} 页、建立 ${result.chunkCount} 个检索片段。`;
...
<template>
  <section class="workspace">
    <div class="workspace-head">
      ...
      <RouterLink :to="`/projects/${projectId}/qa`">进入知识问答</RouterLink>
    </div>
    <p class="subtle">
      当前支持可直接提取文字的
    ...
    <form class="form-stack upload-card" @submit.prevent="submit">
      ...
      <label
        >PDF 文件
        <input accept="application/pdf,.pdf" type="file" @change="selectFile" />
      </label>
