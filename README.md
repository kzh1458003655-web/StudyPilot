...
> 课程版原型已归档。当前分支已完成目标架构基础链路，已完成资料知识库、溯源问答、真题分析与模拟卷生成，已完成在线作答与智能测评，当前进入系统级验证与最终交付。
...
课程版原型只保留在归档分支供回看。当前目标版本以“备考项目”为边界，使用 PostgreSQL Schema、资料驱动组卷和结构化模型评分；完整运行需要本地 C++ 检索服务、模型服务与 PostgreSQL 连接。
