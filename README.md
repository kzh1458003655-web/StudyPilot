...
Agent Harness 已于 2026-09-08 建设并验证完成。本阶段没有执行代码重构、数据库迁移或产品功能开发；下一位 Agent 从 `ARCH-001` 开始。
...
- 支持模拟试卷在线作答和历史作答记录；
- 单项选择题由程序按照标准答案确定性判分；
- 简答题由本地模型结合参考答案、评分要点和资料依据进行辅助评分；
- 保存评分依据、答案解析、错题和测评报告；
...
当前原型与目标产品的主要差异是：原型以“课程”为边界，目标版本以“备考项目”为边界；原型仍使用本地 MySQL，目标后端基线规划为 Supabase PostgreSQL；真题分析、模拟组卷和智能测评仍需按最新产品架构继续完善。
