...
> 课程版原型已归档。当前分支已完成目标架构基础链路，正从资料知识库与溯源问答模块开始实现可交付产品。

## 当前开发阶段

Agent Harness、四项架构任务、数据库任务与基础集成任务已验证完成。课程版原型（含考频、模拟出题和答题评测的最小闭环）已归档；后续从资料知识库与溯源问答开始实现。
...
- 项目删除采用硬删除并级联清理项目内数据，不提供归档、恢复或软删除。
...
当前仓库仍保留早期课程版实现，主要能力包括：

- 课程空间和课程级数据隔离；
- 文字型 PDF 上传、解析、页级文本索引和资料管理；
- 带资料名称和页码的流式问答；
- 本地模型请求代理、推理队列、取消和运行状态；
- 复习计划生成、打卡、调整、完成任务保留和版本记录；
- 课程内考频统计、演示题库、5 题模拟试卷、服务端客观题判分和简答题评分点反馈；
- Java 后端、C++ AI 服务、本地 MySQL 和 Vue 前端的联动启动。

当前原型与目标产品的主要差异是：原型以“课程”为边界，目标版本以“备考项目”为边界；原型仍使用本地 MySQL，目标后端基线规划为 Supabase PostgreSQL；当前考频和评测为离线演示题库闭环，真题自动分析、资料驱动组卷和模型辅助评分仍需按最新产品架构继续完善。
...
当前课程原型可直接演示第 3、6、7、8 步中的资料问答、演示题库组卷和服务端评分；完整真题分析与模型辅助评分属于后续架构任务。
